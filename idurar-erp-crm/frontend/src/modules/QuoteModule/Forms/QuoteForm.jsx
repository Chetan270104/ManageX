...
function LoadQuoteForm({ subTotal = 0, current = null }) {
  ...
  useEffect(() => {
    const currentTotal = calculate.sub(calculate.add(calculate.multiply(subTotal, taxRate), subTotal), discount);
        ...
        <Row gutter={[12, -5]}>
          ...
          <Col className="gutter-row" span={5}>
            <Form.Item
              ...
              initialValue={0}
            >
              <InputNumber style={{ width: '100%' }} min={0} onChange={(value) => setDiscount(value || 0)} />
            </Form.Item>
          </Col>
        </Row>
        <Row gutter={[12, -5]}>
